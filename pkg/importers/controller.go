...
func (n *controller) sync(ctx context.Context, controllerContext factory.SyncContext) error {
	...
	provider, clusterKey, err := provider.ParseKey(key)
	if err != nil {
		return err
	}

	p, ok := n.providers[provider]
	if !ok {
	...
	config := join.BootstrapConfig{
		HubAPIServer: n.hubAPIServer,
		SAName:       "bootstrapper",
		SANamespace:  "open-cluster-management",
	}

	bootstrapper := join.NewTokenBootStrapper(config, n.kubeClient)
	...
	_, err = builder.ApplyImport(ctx, false, false, streams)
